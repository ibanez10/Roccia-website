...
const CheckoutPage = () => {
  ...
  const closeStatusPopup = () => setPaymentStatus(null);

  const handleCheckout = async () => {
    if (!isFormComplete) {
      setPaymentStatus('form-incomplete');
      return;
    }
    ...
    try {
      ...
      if (window.snap) {
        window.snap.pay(token, {
          ...
          onClose: () => setPaymentStatus('closed'),
        });
      } else {
        setPaymentStatus('snap-not-loaded');
      }
    } catch (error) {
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex justify-center items-center">
          <div className="bg-white rounded-xl p-10 max-w-sm text-center shadow-xl">
            {paymentStatus === 'success' && (
              <>
                <h2 className="text-green-600 text-xl font-semibold mb-2">Pembayaran Berhasil!</h2>
                <p className="text-sm text-gray-600">Terima kasih atas pesanan Anda.</p>
              </>
            )}
            {paymentStatus === 'error' && (
              <>
                <h2 className="text-red-600 text-xl font-semibold mb-2">Pembayaran Gagal</h2>
                <p className="text-sm text-gray-600">Silakan coba lagi atau gunakan metode lain.</p>
              </>
            )}
            {paymentStatus === 'closed' && (
              <>
                <h2 className="text-yellow-600 text-xl font-semibold mb-2">Popup Ditutup</h2>
                <p className="text-sm text-gray-600">Kamu menutup popup pembayaran sebelum menyelesaikannya.</p>
              </>
            )}
            {paymentStatus === 'snap-not-loaded' && (
              <>
                <h2 className="text-red-600 text-xl font-semibold mb-2">Snap Tidak Dimuat</h2>
                <p className="text-sm text-gray-600">Pastikan koneksi aman & snap sudah siap digunakan.</p>
              </>
            )}
            {paymentStatus === 'form-incomplete' && (
              <>
                <h2 className="text-red-600 text-xl font-semibold mb-2">Form Belum Lengkap</h2>
                <p className="text-sm text-gray-600">Mohon isi semua informasi pengiriman.</p>
              </>
            )}
            <button
